...
export const renderTaskPrompt = (resolved: ResolvedTask, template: string): string => {
  ...
  const setupNotes = [
    resolved.model ? notes('Model', displayReference(resolved.model.metadata), resolved.model.body) : undefined,
    resolved.harness ? notes('Harness', displayReference(resolved.harness.metadata), resolved.harness.body) : undefined,
  ].filter((value): value is string => Boolean(value));
